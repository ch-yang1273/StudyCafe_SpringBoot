...
class CafeControllerTest {
    ...
    public void 카페를_Follow한다() throws Exception {
        ...
        UserLoginResponseDto dto = userMockMvcHelper.회원가입_후_액세스토큰을_받는다(UserFixture.DAISY);
        cafeMockMvcHelper.카페를_Follow한다(info.getCafeId(), dto.getAccessToken())
                .andExpect(MockMvcResultMatchers.status().isOk());
    }
    ...
    public void 카페를_Unfollow한다() throws Exception {
        ...
        UserLoginResponseDto dto = userMockMvcHelper.회원가입_후_액세스토큰을_받는다(UserFixture.DAISY);
        cafeMockMvcHelper.카페를_Unfollow한다(info.getCafeId(), dto.getAccessToken())
                .andExpect(MockMvcResultMatchers.status().isOk());
    }
    ...
    public void 카페_Follower_정보를_받는다() throws Exception {
        ...
        UserLoginResponseDto dto1 = userMockMvcHelper.회원가입_후_액세스토큰을_받는다(UserFixture.BLOO);
        UserLoginResponseDto dto2 = userMockMvcHelper.회원가입_후_액세스토큰을_받는다(UserFixture.COCO);
        UserLoginResponseDto dto3 = userMockMvcHelper.회원가입_후_액세스토큰을_받는다(UserFixture.DAISY);
        cafeMockMvcHelper.카페를_Follow한다(info.getCafeId(), dto1.getAccessToken());
        cafeMockMvcHelper.카페를_Follow한다(info.getCafeId(), dto2.getAccessToken());
        cafeMockMvcHelper.카페를_Follow한다(info.getCafeId(), dto3.getAccessToken());

        String response = cafeMockMvcHelper.카페_Follower_정보를_받는다(info.getCafeId(), blossom.getAccessToken())
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andReturn()
                .getResponse()
                .getContentAsString();
